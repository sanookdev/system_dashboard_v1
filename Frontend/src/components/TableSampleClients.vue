...
<template>
    ...
    <tbody v-if="items.length > 0">
      <tr v-for="(task, index) in itemsPaginated" :key="task.id">
        ...
        <!-- <td class="border-b-0 lg:w-6 before:hidden">
          <UserAvatar :username="task.name" class="w-24 h-24 mx-auto lg:w-6 lg:h-6" />
        </td> -->
        ...
        <td data-label="User_req_name">{{ task.user_req_name }} ( {{ task.user_req }})</td>
  ...
  <div v-if="items.length > 0" class="p-3 lg:px-6 border-t border-gray-100 dark:border-slate-800">
    <BaseLevel>
      <BaseButtons>
        <BaseButton
